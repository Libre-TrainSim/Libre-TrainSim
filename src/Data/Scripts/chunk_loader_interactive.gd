...
func _get_chunk_file_path(chunk: String):
	var chunk_file := ""
	if Root.Editor:
		chunk_file = chunk_manager.editor.current_track_path.get_base_dir().plus_file("chunks")
	else:
		chunk_file = Root.current_track.get_base_dir().plus_file("chunks")
	chunk_file = chunk_file.plus_file(chunk) + ".tscn"

	if Root.Editor:
		var temp_file = chunk_file.plus_file(chunk) + "_temp.tscn"
