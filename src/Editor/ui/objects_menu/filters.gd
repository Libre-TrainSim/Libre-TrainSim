...
func _ready() -> void:
	for child in get_children():
		child.connect("mouse_entered", Callable(self, "_on_mouse_entered"))
		child.connect("mouse_exited", Callable(self, "_on_mouse_exited"))
...
func set_counter(value: int) -> void:
	if last_frame == Engine.get_process_frames():
		counter = value
		return
