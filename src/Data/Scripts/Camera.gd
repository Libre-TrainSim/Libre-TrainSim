...
@onready var cameraZeroTransform: Transform3D = transform
...
func _input(event) -> void:
	...
	if current and event is InputEventMouseMotion and (not Root.Editor or Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT)) \
			and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		mouseMotion = mouseMotion + event.relative
		mouse_moved = (event.relative != Vector2(0,0))

	if current and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.button_pressed == true:
		saved_mouse_position = get_viewport().get_mouse_position()
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
		mouse_moved = false

	if current and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.button_pressed == false and Root.Editor:
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		get_viewport().warp_mouse(saved_mouse_position)

	if current and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.button_pressed == false and not mouse_moved:
...
func _process(delta: float) -> void:
	...
	if not Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED and not Root.mobile_version and not Root.Editor and not Root.process_mode:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)

	if mouseMotion.length() > 0 and (not Root.Editor or Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT)):
		var motionFactor: float = (refDelta / delta * refDelta) * mouseSensitivity * deg_to_rad(1)
	...
	if accel and player:
		...
		var missingCameraPosition = position.x - sollCameraPosition
		position.x -= missingCameraPosition * delta

	if not fixed and (not Root.Editor or Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT)):
