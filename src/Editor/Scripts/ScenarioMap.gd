...
func create_signal(signal_instance):
	var sprite = get_rail_icon_at(signal_instance.attached_rail, signal_instance.on_rail_position, signal_instance.forward)
...
func update_map():
	...
	Logger.log("Updating scenario map...")
	...
	for route_point in route_data:
		if route_point.type == RoutePointType.STATION:
			...
			sprite = get_rail_icon_at(station.attached_rail, station.on_rail_position, station.forward)
	...
	if spawn_point != {}:
		...
		if spawn_point.type == RoutePointType.STATION:
			...
			sprite = get_rail_icon_at(spawn_point.rail_name, spawn_point.distance, spawn_point.forward)
	...
	if despawn_point.type == RoutePointType.STATION:
		...
		sprite = get_rail_icon_at(spawn_point.rail_name, spawn_point.distance, baked_route.back().forward)
	sprite.name = "DespawnPoint"
	sprite.texture = despawn_point_icon
	$Special.add_child(sprite)


func get_rail_icon_at(rail_name: String, distance: float, forward: bool) -> Sprite:
